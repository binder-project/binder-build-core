...
Builder.prototype._makeDockerfile = function (phase, dir, contents, next) {
  ...
  async.map(_.pairs(contents), function (nameAndContent, cb) {
    var name = nameAndContent[0]
    var content = nameAndContent[1]
    console.log("depName: {0}".format(name))
    ...
    dep.generateString(name, content, _cb)
  }, function (err, depStrings) {
    if (err) {
      phase.error(err)
      return next(err)
    }
...
Builder.prototype._buildDockerfile = function (phase, dir, contents, context, next) {
  ...
  var dockerBuild = function (ctxPath, fCleanup, dCleanup, innerNext) {
    ...
    var params = (this.docker.registry === '') ? [this.docker.username] : [this.docker.registry]
    params.push(this.opts.imageName) 
    ...
    docker.buildImage(ctxPath, buildOpts, function (err, response) {
      ...
      response.on('end', function () {
        ...
        innerNext(null, imageName, fCleanup, dCleanup)
      })
