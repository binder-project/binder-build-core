  ...
  fs.readdir(dir, function (err, files) {
    ...
    var extractFile = function (fileName, cb) {
      ...
      var nameAndContents = function (err, data) {
        ...
        process.exit(2)
        console.error('IN NAMEANDCONTENTS')
        console.log('err: ' + err)
        console.log('data: {0}'.format(JSON.stringify(data)))
        console.log('precedence: {1}'.format(dependencies[fileName].precedence))
    ...
    async.map(depNames, extractFile, function (err, results) {
      ...
      console.log('FINISHED, results: ' + JSON.stringify(results))
      if (err) {
        phase.error(err)
        return next(err)
      }
...
Builder.prototype._selectDependency = function (phase, dir, contents, next) {
  ...
  _.forEach(contents, function (dep, name) {
    if (!selected || dep.precedence < selected.precedence) {
      selected = [name, dep]
      console.log('selected: {0}',format(JSON.stringify(selected)))
    }
  })
  return next(null, selected)
}
...
Builder.prototype._makeDockerfile = function (phase, dir, dep, next) {
  ...
  var depString = (dep) ? dep.generateString(name, contents) : '\n'
  var dockerfile = _constructDockerfile(depString) 
  if (dep) {
    ...
    return next(null, dir, dockerfile, [])
  }
}
